...
# Ensure the ERA column exists and clean the data
if "ERA" in df.columns and "WIN %" in df.columns:
    # Remove non-numeric or missing values in ERA and WIN %
    df = df[pd.to_numeric(df["ERA"], errors="coerce").notnull()]
    ...
    # Plot the trendline
    plt.plot(x, trendline(x), color='red', label=f'Trendline: Win % = {coefficients[0]:.4f} * ERA + {coefficients[1]:.4f}')

    # Add labels and title
    plt.xlabel("ERA (Lower is Better)", fontsize=12)
    plt.ylabel("Win %", fontsize=12)
    plt.title("Win % vs. ERA with Trendline", fontsize=14)
    plt.legend()
    plt.grid(True)

    # Save and show the plot
    output_plot_file = "win_vs_era_trendline.png"
    plt.savefig(output_plot_file, dpi=300)
    plt.show()

    print(f"Plot saved as {output_plot_file}")
else:
    print("The required columns 'ERA' and/or 'WIN %' are not present in the dataset.")
